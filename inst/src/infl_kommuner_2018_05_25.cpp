...
int main(int nargs, char ** argsv){
  ...
  beta = atof(argsv[1]);
  gamma = 1/atof(argsv[2]);
  a = 1/atof(argsv[3]);
  dailySeeding = atoi(argsv[4]);
  Graph G;
  string tmpstr;
  string tmpstr2;
  char tmpchar[3];
  ...
  for(int i_sim = 0; i_sim < N; ++i_sim){
    ...
    for(int i_day = 0; i_day < M; ++i_day){
      ...
      */
      //if(i_day<M/2){
        if(G_current.locations[40].S > dailySeeding){  // Number 40 is Oslo
          G_current.locations[40].I += dailySeeding;
          G_current.locations[40].S -= dailySeeding;
        }
      //}
