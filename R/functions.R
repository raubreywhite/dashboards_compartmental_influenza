...
SetupCPPAndStructure <- function(){
  ...
  mixingMatrix <- matrix(NA,nrow=4,ncol=4)
  colnames(mixingMatrix) <- row.names(mixingMatrix) <- c("0-4","5-19","20-64","65+")
  mixingMatrix[1:4,1:4] <- 0.5
  diag(mixingMatrix) <- 1
  ...
  pop_wo_com <- pop_wo_com[,.(pop=round(sum(pop)*popMultiplier)),by=.(location)]
  di_edge_list <- di_edge_list[,.(n=round(sum(n)*popMultiplier)),by=.(from,to)]
